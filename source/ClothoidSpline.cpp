...
ClothoidSpline::ClothoidSpline(const vector <Vector> & edgePoints, float penalty, bool using_G3,
	...
	//1 - Curvature estimation:	
        ClothoidSpline::getCurvatures(edgePointSet, estCurv);
...
void ClothoidSpline::getCurvatures(const vector <Vector> & theEdgePointSet, vector <float> & curvVec) {
	...
	for (int i=1;i<theEdgePointSet.size()-1;i++) {
		...
		//this curvature definition from schneider/kobbelt paper
                const Vector vec1=theEdgePointSet[i]-theEdgePointSet[i-1];
                const Vector vec2=theEdgePointSet[i+1]-theEdgePointSet[i];
