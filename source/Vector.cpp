...
Vector Vector::GetRotatedX(float angle)
{
...
Vector Vector::GetRotatedY(float angle)
{
   float sinAngle = (float)sinf(M_PI * angle / 180);
        float cosAngle = (float)cosf(M_PI * angle / 180);
...
Vector Vector::GetRotatedYRadians(float radians) {
        float sinRadians=(float)sinf(radians);
        float cosRadians=(float)cosf(radians);

	return Vector(x * cosRadians + z * sinRadians, y, -x * sinRadians + z * cosRadians);
}

Vector Vector::GetRotatedZ(float angle)
{
   float sinAngle = (float)sinf(M_PI * angle / 180);
        float cosAngle = (float)cosf(M_PI * angle / 180);
...
Vector Vector::GetRotatedZRadians(float radians) {
        float sinRadians=(float)sinf(radians);
        float cosRadians=(float)cosf(radians);
...
Vector Vector::GetRotatedAxis(float angle, Vector axis)
{
   ...
   float newAngle = M_PI * angle / 180;
        float sinAngle = (float)sinf(newAngle);
        float cosAngle = (float)cosf(newAngle);
...
float Vector::intersect(Vector pOrigin, Vector pNormal, Vector rOrigin, Vector rVector)
{
    float d = -(pNormal.DotProduct3(pOrigin));
    float numer = pNormal.DotProduct3(rOrigin) + d;
    float denom = pNormal.DotProduct3(rVector);
    return -(numer / denom);
}

float Vector::intersectEllipse(Vector rO, Vector rV, Vector sO, Vector sR)
{
    Vector Q = sO - rO;
    float c = Q.GetLength();
    float v = Q.DotProduct3(rV);
    float d = sR.DotProduct3(sR)-(c*c-v*v);
