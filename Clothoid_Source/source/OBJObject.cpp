...
void OBJObject::load(char *directory, char *filename) {
    ...
    */
	...
	sprintf_s(objfilename,OBJFILENAMELEN,"%s/%s",directory,filename);
    fopen_s(&openfilehandle,objfilename,"rb");
    ...
    while (curfileindex<filelength) {     
        ...
        if (strncmp(&filecontents[curfileindex],"mtllib",6)==0) {
            ...
            sprintf_s(fullmtlfilename,MAXFILENAMELEN,"%s/%s",directory,mtlfilename);
            
            fopen_s(&mtlfilehandle,fullmtlfilename,"rb");
            ...
            strcpy_s(texturefilename,MAXFILENAMELEN,&eachline[curmtllineindex]);
            
            char *fulltexturefilename=new char[MAXFILENAMELEN];
            sprintf_s(fulltexturefilename,MAXFILENAMELEN,"%s/%s",directory,texturefilename);                     
            ...
            */
            textureindex=ilutGLLoadImage(fulltexturefilename);
